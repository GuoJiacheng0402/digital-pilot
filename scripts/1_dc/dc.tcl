# DigitalPilot · DC 综合主脚本(最终可行版原样收录;设计名由 define.tcl 提供)
# ################################################################################
...
# Single compile_ultra pass. Trying compile_ultra on v5 RTL — the mults are
# smaller and the divider is spread across 6 pipeline stages, so the resulting
# netlist may route better than v6.x compile_ultra.
...
report_timing -delay_type min -sig 4 -cap -transition_time -nets -attributes -nosplit > ./output/${DESIGN_NAME}.timing_min.rpt
report_timing -delay_type max -sig 4 -cap -transition_time -nets -attributes -nosplit > ./output/${DESIGN_NAME}.timing.rpt
report_timing -delay_type max -max_paths 10 -sig 4 -nosplit > ./output/${DESIGN_NAME}.timing_top10.rpt
